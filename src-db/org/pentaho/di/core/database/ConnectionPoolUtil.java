...
public class ConnectionPoolUtil
{
    ...
    private static void createPool(LogChannelInterface log, DatabaseMeta databaseMeta, String partitionId, int initialSize, int maximumSize) throws KettleDatabaseException
    {
        ...
        //xnren start
        //set autoCommit as user defined
        boolean autoCommit = true;
        boolean readOnly = false;
        if(properties.get("defaultAutoCommit") != null && !"true".equals(properties.get("defaultAutoCommit").toString())){
        	autoCommit = false;
        }
        if(properties.get("defaultReadOnly") != null && "true".equals(properties.get("defaultReadOnly").toString())){
        	readOnly = true;
        }
        
        new PoolableConnectionFactory(cf, gpool, null, null, readOnly, autoCommit);
