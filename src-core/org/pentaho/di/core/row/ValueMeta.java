...
//    }
    ...
    //the original method is commented!
    //this method try different common date format!  jason
    private synchronized Date convertStringToDate(String string) throws KettleValueException
    {
        string = Const.trimToType(string, getTrimType()); // see if  trimming needs to be performed before conversion
        ...
        try
        {
            date = getDateFormat().parse(string);
            return date;
        }
        catch (ParseException e)
        {}
        ...
        if (date==null)
        {
        	...
        	try {
				...
				String dateFormatStr = (getDateFormat() != null) ? getDateFormat().toPattern() : "null";
	            throw new KettleValueException(toString()+" : couldn't convert string ["+string+"] to a date using format ["+dateFormatStr+"]" +" and format yyyy-MM-dd HH:mm:ss.SSS,  and format yyyy-MM-dd HH:mm:ss and format yyyy-MM-dd" );				
			}
        }
    ...
    public synchronized SimpleDateFormat getDateFormat()
    {
        ...
        if (dateFormat==null || dateFormatChanged)
        {
            ...
            if (dateFormatLocale==null || dateFormatLocale.equals(Locale.getDefault()))
            {
                ...
                //dateFormat = new SimpleDateFormat(mask,new Locale("en"));
            }
            else
            {
    ...
    public String getString(Object object) throws KettleValueException
    {
        try
        {
            ...
            case TYPE_BIGNUMBER:
                ...
                {
//                STRAT JYHE 20121213
                case STORAGE_TYPE_NORMAL:         try{string = convertBigNumberToString((BigDecimal)object);}catch(Exception e){string=convertNumberToString((Double)object);} break;
                case STORAGE_TYPE_BINARY_STRING:  try{string = convertBigNumberToString((BigDecimal)convertBinaryStringToNativeType((byte[])object));}catch(Exception e){string=convertNumberToString((Double)object);} break;
                case STORAGE_TYPE_INDEXED:        try{string = object==null ? null : convertBigNumberToString((BigDecimal)index[((Integer)object).intValue()]);} catch(Exception e){string=convertNumberToString((Double)object);}break;
//                END JYHE 20121213
                default: throw new KettleValueException(toString()+" : Unknown storage type "+storageType+" specified.");
                }
                break;
    ...
    public byte[] getBinaryString(Object object) throws KettleValueException
    {
        ...
        try
        {
            ...
            case TYPE_INTEGER:
                switch(storageType)
                {
                case STORAGE_TYPE_NORMAL:         return convertStringToBinaryString(convertIntegerToString(Long.parseLong(object.toString())));
